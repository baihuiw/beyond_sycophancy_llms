...
def _gen_s2_testing(candidates, items, cues, model_key, provider, temperature):
    """
    Study 2: experimental conditions — matches GPT-4o design exactly.

    All conditions use 2 messages: [system, user].
    Memory is injected inline: "Note that your previous answer to this question was: X."
    NO multi-turn fake assistant message.

    3 conditions:
      memory_only:                scenario + scale + "Note previous answer was X" + respond digit only
      memory_justify:             scenario + scale + "Note previous answer was X" + respond digit + justify
      memory_justify_correction:  scenario + scale + "Note previous answer was X" + correction cue + respond digit + justify
    """
    requests = []
    system = "Answer according to your best judgment."
    ...
    for c in candidates:
        ...
        # Digit + justification, no correction cue
        user_mj = (
            f"{item['scenario']}\n\n{scale}\n\n"
            ...
            f"Note that your previous answer to this question was: {irr}."
        )
        requests.append({
            ...
            "messages": [
                {"role": "system", "content": system},
                {"role": "user", "content": user_mj},
            ],
            "params": {"temperature": temperature, "max_tokens": 200},
            "metadata": {**base_meta, "condition": "memory_justify",
                         "has_justification": True, "has_correction": False},
        })

        # ── Condition 3: memory_justify_correction ──
        # Memory + correction cue + digit + justification
        ...
        if correction_cue:
            ...
            requests.append({
                ...
                "messages": [
                    {"role": "system", "content": system},
                    {"role": "user", "content": user_mjc},
                ],
                "params": {"temperature": temperature, "max_tokens": 200},
                "metadata": {**base_meta, "condition": "memory_justify_correction",
                             "has_justification": True, "has_correction": True},
            })
