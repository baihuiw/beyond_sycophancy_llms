...
def peer_system_prompt(role: str, target_position: int) -> str:
    return (
        f"You are Agent {role} (one of A2/A3/A4) in a small group discussion about a moral dilemma.\n"
...
def last_peer_speaker(transcript: List[Dict]) -> Optional[str]:
...
def simulate_episode(
    ...
    Design:
      ...
      - A2/A3/A4 speak next (randomized order), conversational + cross-peer references, fixed positions
    ...
    sys_peers = {}
    for name, role, ppos in zip(PEER_NAMES, peer_roles, peer_positions):
        ...
        for token in order:
            if token == "A1":
                ...
                final_user_msg = (
                    ...
                    "- 1 brief sentence summarizing key points from A2/A3/A4 (mention at least two agents).\n"
            ...
            fixed_pos = peer_positions[PEER_NAMES.index(token)]
            required_digit = fixed_pos

            prev_peer = last_peer_speaker(transcript)
...
def main():
    ...
    ap.add_argument("--schedule", default="A2,A3,A4,A1")
